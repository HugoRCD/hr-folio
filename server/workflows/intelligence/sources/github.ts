...
export async function githubWorkflow(date: string) {
  ...
  const data = await fetchSourceData(mcp, 'Fetch all my GitHub activity for today: commits pushed, pull requests opened/merged/reviewed, issues created/closed, and any other notable activity.')
